...
| `api_url` | `https://api.mem0.ai` | Base URL of the Mem0 Platform API. Point it at a self-hosted or dedicated deployment. `MEM0_API_URL` takes precedence. |
| `mcp_url` | derived from `api_url` | MCP endpoint used by the optional `mem0-platform` server. Defaults to `https://mcp.mem0.ai/mcp/`, or to `<api_url>/mcp/` when a self-hosted `api_url` is set. `MEM0_MCP_URL` takes precedence. |

## Self-hosted Mem0

Point the plugin at your own Mem0 deployment by setting the API base URL:

```bash
export MEM0_API_URL=http://localhost:8888
export MEM0_API_KEY=<your server's key>
```

Every memory the plugin creates or searches uses that base URL. The `api_url`
setting does the same thing without an environment variable. Both `MEM0_API_URL`
and `MEM0_MCP_URL` must be a full `http://` or `https://` URL with a host; a
value that is set but malformed is an error rather than a silent fall back to
`api.mem0.ai`.

The default `mem0` MCP server is local and read-only. To let the coding agent
call the full Platform tool set over MCP — `add_memory`, `update_memory`,
`delete_memory`, `get_memories`, `list_entities`, and the rest — the extra
`mem0-platform` server points at your deployment's MCP endpoint. A self-hosted
server serves MCP at `/mcp/` on the same origin as its REST API, so it follows
`MEM0_API_URL` automatically and needs no extra setting. Set `MEM0_MCP_URL` (or
the `mcp_url` setting) only when MCP lives elsewhere, such as behind a gateway:

```bash
export MEM0_MCP_URL=http://localhost:8888/mcp/
```

Its tools appear as `mcp__mem0-platform__*`. The `mcp_url` setting is the
equivalent, and `mem0-platform` runs through
[`uv`](https://docs.astral.sh/uv/) because the proxy declares its `mcp`
dependency inline:
...
local server and the hooks keep working.
...
- **Env vars still work.** `MEM0_API_KEY`, `MEM0_USER_ID`, `MEM0_PROJECT_ID`, `MEM0_API_URL`, `MEM0_MCP_URL`.
- **API keys are no longer read from shell profiles.** The old plugin could extract `MEM0_API_KEY` from `~/.zshrc` or `~/.bashrc`. Export it, or set the `api_key` plugin setting, before starting Claude Code.
