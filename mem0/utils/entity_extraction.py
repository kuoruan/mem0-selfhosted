...
Extracts entity candidates from a spaCy-processed document:
- **Proper nouns / named entities**: spaCy NER (CJK / ``ner`` mode) or capitalization
  heuristics (Latin / ``heuristic`` mode)
- **Quoted text**: text in single/double quotes, including CJK quote forms (「」 “” ‘’)
- **Topic phrases**: multi-word noun phrases with specific modifiers (e.g., "machine learning")
- **Identifiers**: technical dotted identifiers (package names, API paths)

Public API:
    ``extract_entities(text, *, nlp_config=None)`` accepts a string and owns spaCy model
    loading. Pass an ``NlpConfig`` to control language, model, and extraction mode.
    ``extract_entities_batch(texts, *, nlp_config=None)`` uses ``nlp.pipe`` for batched
    extraction.
...
def _add_candidate(
    ...
    *,
    ...
    if not cleaned or len(cleaned) < min_len or _has_artifacts(cleaned):
        return
    candidates.append(
        _EntityCandidate(
...
def _add_ner_candidates(doc, candidates: list[_EntityCandidate], *, min_len: int = 3) -> None:
    tokens = list(doc)
    for ent in doc.ents:
...
def _add_technical_identifier_candidates(
...
def _add_proper_name_candidates(
    ...
    while i < len(tokens):
        ...
        if len(name_tokens) > 1 or not _is_bad_single_name_token(name_tokens[0]):
            ...
            _add_candidate(candidates, "PROPER", text, "proper_name_span", i, j, 0.8, 2, min_len=min_len)
        i = max(j, i + 1)


def _add_quoted_candidates(text: str, candidates: list[_EntityCandidate], *, min_len: int = 3) -> None:
    for m in _RE_QUOTED_DQ.finditer(text):
        val = next((g for g in m.groups() if g is not None), "").strip()
        if val and len(val) >= min_len:
            _add_candidate(candidates, "QUOTED", val, "quoted", -1, -1, 0.75, 3, min_len=min_len)
...
def _add_topic_phrase_candidates(
    doc, candidates: list[_EntityCandidate], *, min_len: int = 3, is_cjk: bool = False
) -> None:
    # Some spaCy pipelines (notably CJK models without a parser) do not implement
    # noun_chunks; treat that as "no topic phrases" rather than crashing.
    try:
            ...
            if compound_toks:
                ...
                if is_circ:
                    ...
                    if filtered:
                        ...
                        if len(phrase) > min_len and (is_cjk or " " in phrase):
                            _add_candidate(
                                candidates,
                                "TOPIC",
                                phrase,
...
def _extract_entities_from_doc(
    ...
    *,
    ...
    candidates: list[_EntityCandidate] = []
    if use_ner:
        _add_ner_candidates(doc, candidates, min_len=min_len)
...
def extract_entities(
    ...
    *,
    ...
    Args:
        ...
        nlp_config: Optional ``NlpConfig`` controlling language/model/extraction mode.
            When omitted, a default English config is used.
    """
