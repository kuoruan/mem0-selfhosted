...
| `api_url` | `https://api.mem0.ai` | Base URL of the Mem0 Platform API. Point it at a self-hosted or dedicated deployment. The `MEM0_API_URL` environment variable takes precedence. |
| `mcp_url` | derived from `api_url` | MCP endpoint used by the optional `mem0-platform` server (see [Self-hosted Mem0](#self-hosted-mem0)). Defaults to `https://mcp.mem0.ai/mcp/` on the platform, and to `<api_url>/mcp/` when a self-hosted `api_url` is set. The `MEM0_MCP_URL` environment variable takes precedence. |

## Self-hosted Mem0

The plugin talks to the Mem0 Platform API. Point it at your own deployment by
setting the API base URL before starting Claude Code:

```bash
MEM0_API_URL=http://localhost:8888
MEM0_API_KEY=<your server's key>
```

Every memory the plugin creates or searches — hooks, background flush, and the
`search_memories` MCP tool — uses that base URL. You can also set the same value
as the `api_url` plugin setting instead of an environment variable.

Both `MEM0_API_URL` and `MEM0_MCP_URL` must be a full `http://` or `https://`
URL with a host. A value that is set but malformed is an error, not a silent
fall back to `api.mem0.ai`, so a typo cannot send your key to the public
endpoint.

The `mem0-platform` server follows `MEM0_API_URL` automatically: a self-hosted
server serves MCP at `/mcp/` on the same origin as its REST API, so setting only
`MEM0_API_URL` is enough. Set `MEM0_MCP_URL` only when MCP lives somewhere else,
such as behind a gateway.

### Optional: full Platform tool set over MCP

The default `mem0` MCP server is local and read-only: it exposes one
`search_memories` tool that queries the API above. Two-way conversations happen
through the hooks.

If you want the coding agent to call the full Platform tool set directly —
`add_memory`, `update_memory`, `delete_memory`, `get_memories`, `list_entities`,
and the rest — the plugin also ships a `mem0-platform` MCP server that proxies
the remote MCP endpoint:

```bash
export MEM0_MCP_URL=http://localhost:8888/mcp/   # defaults to https://mcp.mem0.ai/mcp/
```

Set `mcp_url` as a plugin setting for the same effect. Its tools appear as
`mcp__mem0-platform__*` alongside the local `mcp__mem0__search_memories`.

This server runs through [`uv`](https://docs.astral.sh/uv/), because the proxy
declares its `mcp` dependency inline:

```bash
curl -LsSf https://astral.sh/uv/install.sh | sh
```

Without `uv` on `PATH`, only the `mem0-platform` server fails to start; the
local `mem0` server and the hooks are unaffected. Disable it from `/mcp` if you
do not want the extra tool set. (Marketplace installs manage `.mcp.json` for
you, so editing the file is not the way to turn it off.)

> The proxy forwards the endpoint's own tool names and schemas, which come from
your server. They are not the same as the local `search_memories` tool's
`scope`/`category` arguments, so the generated skills keep using the local server.
...
- **API keys come from the plugin setting or the environment.** The old plugin could extract `MEM0_API_KEY` from your shell profile files. The current plugin resolves it from `MEM0_API_KEY`, the host's plugin config, or a key file in the plugin's data directory — it does not read `~/.zshrc` or `~/.bashrc`. Export the key, or set it as the `api_key` plugin setting, before starting the host.
...
- **MCP server replaced.** Nine read/write tools replaced by the single read-only `search_memories` tool. The old tool set is available again as the opt-in `mem0-platform` server — see [Self-hosted Mem0](#self-hosted-mem0).
...
- **Most `MEM0_*` env vars ignored.** Only `MEM0_API_KEY`, `MEM0_USER_ID`, `MEM0_RESOLVED_USER_ID`, `MEM0_PROJECT_ID`, `MEM0_API_URL`, and `MEM0_MCP_URL` are still read. Run `/mem0:status` to see what is active.
