...
via ``Authorization: Token <api key>``.

Self-hosted deployments point ``MEM0_MCP_URL`` (or the host's plugin option) at
their own MCP endpoint, which exposes the full platform tool surface. This is
opt-in: the default ``mem0`` MCP server shipped in each plugin is the local
search-only ``mcp_server.py``. It runs through ``uv`` because of the inline
...
from memory_core import api_key, mcp_url

logger = logging.getLogger("mem0.mcp_proxy")


def _auth_headers() -> dict[str, str]:
    """Return ``Authorization: Token <api key>`` resolved by the shared identity chain."""
    key = api_key()
...
async def main() -> None:
    ...
    async with streamablehttp_client(mcp_url(), headers=_auth_headers()) as (read, write, _get_sid):
