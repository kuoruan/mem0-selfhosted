...
def _configured_url(names: tuple[str, ...]) -> str:
    """Return the first explicitly set URL, or "" when none is set.

    A value that is set but unusable raises instead of falling back to the public
    default, so a typo cannot silently send credentials to a host the operator did
    not configure. Callers surface the error instead of degrading quietly.
    """
    for name in names:
        ...
        if _valid_http_url(value):
            return value
        raise ValueError(f"{name} must be an http:// or https:// URL with a host (got {value!r})")
    return ""


def _resolve_url(names: tuple[str, ...], default: str) -> str:
    """Return the first explicitly set URL, or *default* when none is set."""
    return _configured_url(names) or default


def api_url() -> str:
    """Resolve the Platform/REST base URL (env first, then host plugin option).

    Trailing slashes are dropped because callers append absolute paths.
    """
    return _resolve_url(
        ("MEM0_API_URL", "PLUGIN_OPTION_API_URL", "CLAUDE_PLUGIN_OPTION_API_URL"),
        DEFAULT_API_URL,
    ).rstrip("/")


def mcp_url() -> str:
    """Resolve the remote MCP endpoint used by ``mcp_proxy.py``.

    ``MEM0_MCP_URL`` (or the host plugin option) wins. When only a self-hosted
    ``api_url`` is configured, the endpoint is derived from it, because a
    self-hosted server mounts MCP at ``/mcp/`` on the same origin as its REST API.
    Using the public default in that case would send the self-hosted key to Mem0,
    so the public default is only used when ``api_url`` is the public one too.
    Returned verbatim: an MCP endpoint's trailing slash is meaningful.
    """
    configured = _configured_url(
        ("MEM0_MCP_URL", "PLUGIN_OPTION_MCP_URL", "CLAUDE_PLUGIN_OPTION_MCP_URL")
    )
    if configured:
        return configured
    base = api_url()
    if urllib.parse.urlsplit(base).netloc != urllib.parse.urlsplit(DEFAULT_API_URL).netloc:
        return base + "/mcp/"
...
def flush_session(
    ...
    base_url = api_url()
    add_url = f"{base_url}/v3/memories/add/"
...
def search_memories(
    ...
    *,
    ...
    url = api_url() + "/v3/memories/search/"
    started = time.perf_counter()
    try:
...
def forget_remote_repo(
    ...
    base_url = api_url()
    try:
        memory_ids = _scoped_memory_ids(base_url, key, user, repo, include_project_memory)
    except Exception as exc:
        telemetry.record(
            "forget", repo=repo, success=False, error_kind=telemetry.error_kind(exc)
        )
        return {"status": "error", "error": bounded(str(exc), 1000)}
    deleted = sum(_delete_memory(base_url, key, memory_id) for memory_id in memory_ids)
