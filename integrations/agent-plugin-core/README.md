...
`memory_core.api_url()` resolves the Platform/REST base URL and `memory_core.mcp_url()` resolves the remote MCP endpoint. `api_url()` follows — `MEM0_API_URL`, then the host's `PLUGIN_OPTION_API_URL` and `CLAUDE_PLUGIN_OPTION_API_URL`, then `https://api.mem0.ai`. `mcp_url()` follows the same chain for `MEM0_MCP_URL`; when nothing is set it derives `<api_url>/mcp/` for a self-hosted `api_url` (a self-hosted server mounts MCP at `/mcp/` on the same origin as its REST API) and `https://mcp.mem0.ai/mcp/` otherwise, so a self-hosted key is never offered to the public MCP endpoint. A value that is set but is not an `http`/`https` URL with a host raises `ValueError` instead of falling back to the public default. `api_url()` drops a trailing slash; `mcp_url()` keeps it. Self-hosted and dedicated deployments set these to their own server. Both are read on every call, so they must be set before the plugin starts.

`python/mcp_proxy.py` uses `mcp_url()`. It is a stdio→streamable-http proxy that forwards an editor to a remote MCP endpoint, giving agents the endpoint's full tool surface. The Claude Code, Cursor, Codex, and Antigravity plugins register it as an additional `mem0-platform` server next to the local `mcp_server.py`, which stays the default `mem0` server in every host. The file is bundled into all six plugins because `core/` is generated as a whole; Kimi, the portable package, and hosts without MCP configuration ship it without a registration. The proxy declares `mcp>=1.25.4` in PEP 723 inline metadata and is launched through `uv`, so it is the only part of the bundle with a runtime dependency; a host without `uv` fails to start only that one server.
